        ...
        array(
            ...
            'buttons'=>array(
                ...
                '_solution_update' => array(
                    ...
                    'visible'=>'(Solution::model()->find(\'challenge_id = :challenge_id and player_id = :player_id\', array(\':challenge_id\'=>$data->id, \':player_id\'=>User::model()->findByPk(Yii::app()->user->id)->player->id))->completed == 0) && (date(\'Y-m-d H:i:s\') <= date(\'Y-m-d H:i:s\', strtotime($data->deadline))) && (count(Solution::model()->find(\'challenge_id = :challenge_id and player_id = :player_id\', array(\':challenge_id\'=>$data->id, \':player_id\'=>User::model()->findByPk(Yii::app()->user->id)->player->id))) > 0)',
                ),
