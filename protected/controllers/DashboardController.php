...
class DashboardController extends Controller
{
	...
	public function actionIndex()
    {
        ...
        $dashboard['leaderboardPosition'] = (int) Yii::app()->db->createCommand('select t.position from (select p.id, @rownum := @rownum + 1 as position from players p, (select @rownum := 0) r order by p.first_currency + p.second_currency desc) as t where t.id = '.$player->id)->queryRow();
